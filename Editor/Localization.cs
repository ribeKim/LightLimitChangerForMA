...
이 기능은 실행 시 비파괴로 머티리얼 텍스처를 변경하기 때문에 잠재적인 버그가 발생할 수 있으며, 텍스처 메모리가 증가할 수 있습니다.
...
另外，通过使用TexTransTool进行图集，可以显着减少纹理内存的增加。",
                    @"lilToon의 경우, 사전에 텍스처를 구워 텍스처 메모리가 증가하는 것을 줄일 수 있습니다.
...
这可能会导致动画更改不相关的属性。",
                @"{0}개의 객체(렌더러) 내에서 제외되거나 해당되지 않는 셰이더가 사용되고 있습니다.
...
如果你不是有意的，請檢查你的設定。",
                    @"菜单未生成，因为没有动画目标。
如果你不是有意的，请检查你的设置。",
                    @"애니메이션을 생성할 대상이 존재하지 않아 메뉴를 생성하지 않았습니다.
...
为 Quest 构建时，此错误是正常的，因为不支持 Quest 着色器。",
                    @"lilToon/Poiyomi/Sunao 셰이더만 지원됩니다.
